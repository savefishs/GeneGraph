...
class TranSiGen(torch.nn.Module):
    def __init__(self, n_genes, n_latent, n_en_hidden, n_de_hidden, features_dim, features_embed_dim, **kwargs):
        ...
        self.proj = nn.Linear(978, 256)  # or 256
        ...
        self.classifier = nn.Sequential(
                ...
                nn.Linear(512, 128),
                nn.ReLU(),
                nn.Linear(128, 1)   # n_classes 需要你传进来
            )
    ...
    def forward(self, x1, features_A, features_B):
        ...
        if self.feat_embed_dim != None:
            feat_embed_A = self.feat_embeddings(features_A)
            feat_embed_B = self.feat_embeddings(features_B)
        else:
            feat_embed_A = features_A
            feat_embed_B = features_B
        z1_feat_A = torch.cat([z1, feat_embed_A], 1)
        z1_feat_B = torch.cat([z1, feat_embed_B], 1)
        
        mu_pred_A, logvar_pred_A = self.mu_z2Fz1(z1_feat_A), self.logvar_z2Fz1(z1_feat_A)
        mu_pred_B, logvar_pred_B = self.mu_z2Fz1(z1_feat_B), self.logvar_z2Fz1(z1_feat_B)

        z2_pred_A = self.sample_latent(mu_pred_A, logvar_pred_A)
        z2_pred_B = self.sample_latent(mu_pred_B, logvar_pred_B)

        x2_pred_A = self.decode_x2(z2_pred_A)
        x2_pred_B = self.decode_x2(z2_pred_B)

        
        A_proj = self.proj(x2_pred_A)  # [batch, 128]
        B_proj = self.proj(x2_pred_B)  # [batch, 128]

        A_seq = A_proj.unsqueeze(1)
        B_seq = B_proj.unsqueeze(1) 

        # cross-attention
        A_attn, _ = self.cross_attention(A_seq, B_seq, B_seq)  # A attends to B
        B_attn, _ = self.cross_attention(B_seq, A_seq, A_seq)  # B attends to A

        A_attn = A_attn.squeeze(1)
        B_attn = B_attn.squeeze(1)

        # --- 拼接 原始 + 交互特征 ---
        combined = torch.cat([A_proj, B_proj, A_attn * B_attn], dim=-1)
    ...
    def train_model(self, learning_rate, weight_decay, n_epochs, train_loader, test_loader, save_model=True, metrics_func=None):
        ...
        for epoch in range(n_epochs):
            ...
            log_file_path = os.path.join(self.path_model,"train_log.txt")
            print_str = (
                f"[Epoch {epoch}] | loss: {train_loss:.3f}, mse_x1_rec: {train_mse_x1:.3f}, mse_x2_rec: {train_mse_x2:.3f}, mse_pert: {train_mse_pert:.3f}, "
                f"kld_x1: {train_kld_x1:.3f}, kld_x2: {train_kld_x2:.3f}, kld_pert: {train_kld_pert:.3f} | "
                f"valid_loss: {test_loss:.3f}, valid_mse_x1_rec: {test_mse_x1:.3f}, valid_mse_x2_rec: {test_mse_x2:.3f}, valid_mse_pert: {test_mse_pert:.3f}, "
                f"valid_kld_x1: {test_kld_x1:.3f}, valid_kld_x2: {test_kld_x2:.3f}, valid_kld_pert: {test_kld_pert:.3f}"
)
